...
const main = async () => {
  ...
  // 1. Function to display the percentage breakdown of liquidity sources
  console.log("############################\n############################");
  console.log(quote.route.fills.length, " Sources");
  quote.route.fills.forEach((fill: any) => {
    const percentage = (parseInt(fill.proportionBps) / 100).toFixed(2);
    console.log(`${fill.source}: ${percentage}%`);
  });
  console.log("Integrator fees: ", quote.fees.integratorFee.amount);
  console.log("zeroEx fees: ", quote.fees.zeroExFee.amount);

  // 3. Display Buy/Sell Token Tax
  console.log(`Buy Token Buy Tax: ${quote.tokenMetadata.buyToken.buyTaxBps}%`);
  console.log(`Buy Token Sell Tax: ${quote.tokenMetadata.buyToken.sellTaxBps}%`);
  console.log(`Sell Token Buy Tax: ${quote.tokenMetadata.sellToken.buyTaxBps}%`);
  console.log(`Sell Token Sell Tax: ${quote.tokenMetadata.sellToken.sellTaxBps}%`);
  console.log("############################\n############################");
...
// 4. Display all liquidity sources on Scroll
  ...
  const sourceResponse = await fetch(
    "https://api.0x.org/sources?" + sourceParams.toString(),
    {
      headers,
    }
  );
  ...
  sources.sources.forEach((source: any) => {
    if(sources.sources[sources.sources.length-1] === source){
      console.log(`${source}`)
     }
